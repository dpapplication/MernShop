...
interface Order {
  ...
  payments?: {
    ...
    typePaiement: string;
    montant: string; // Always a string
  }[];
  remiseGlobale?: string;
  createdAt?: string;  // Add for displaying
}
...
const calculateTotalPayments = (payments?: Order['payments']): string => {
    if (!payments) return '0.00';
      let total = 0;
...
const NewOrderPage: React.FC = () => {
  ...
  // --- State for New Product ---
    ...
    const [newPayment, setNewPayment] = useState<{ typePaiement: string; montant: string }>({
        typePaiement: '', // Initialize as empty string, not null
        montant: '',       // Initialize as empty string
    });
   ...
   const handleNewPaymentTypeChange = (value) => {
    ...
    event.preventDefault();  
      // Validation
        if (!newPayment.typePaiement || !newPayment.montant) {
            toast({ title: "Error", description: "Fill type, and amount.",  variant: "destructive",});
        return;
      }
        if (isNaN(parseFloat(newPayment.montant))) {
            toast({  title: "Error", description: "Invalid amount format.", variant: "destructive" });
        return;
        }
      ...
      const paymentToAdd = {
          ...
          montant: newPayment.montant, // Keep as string
      };
    ...
    setNewPayment({ typePaiement: '', montant: '' });
  };
...
const produits=order.produits.map(e=>({produit:e.produit._id,prix:e.produit.prix,quantite:e.quantite,remise:e.remise}))
          try {
            ...
            navigate('/Commande');  // Go to the orders list
      ...
      <div className='container mx-auto py-8'>
        <Card>
           <CardHeader>
                ...
                <CardDescription>
                  Ajouter Informations Client , Produits , et  paiements .
                </CardDescription>
            </CardHeader>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <div className="grid grid-cols-2 gap-4 mb-4">
                                    ...
                                    <Select
                                       onValueChange={handleNewPaymentTypeChange}
